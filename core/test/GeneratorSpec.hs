  ...
  spec = do
    describe "Simple expression" $ do
      it "Generates fixed count of fixed ranges" $ do
        generate' "evendistr 1 1" `shouldReturn` [1]
        generate' "evendistr 2 1" `shouldReturn` [1, 1]
        generate' "evendistr [1..1] [1..1]" `shouldReturn` [1]

    describe "Even random expression" $ do
      it "Generates even distribution of values" $ do
        avg <$> generate' "evendistr 1000 [0..100]" >>= (`shouldSatisfy` (< 3) . abs . (50 -))
